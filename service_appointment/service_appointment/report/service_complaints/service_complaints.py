...
def get_data(filters=None):
	...
	for row in result:
		...
		if row.related_to:
			complaint_date = frappe.get_value('Service Appointment', row.related_to, 'date')
			complaint_status = frappe.get_value('Service Appointment', row.related_to, 'appointment_status')
			completed_by = frappe.get_value('Service Appointment', row.related_to, 'completed_by')
			service_type = frappe.get_value('Service Appointment', row.related_to, 'service_type')
			service_amount = frappe.get_value('Service Appointment', row.related_to, 'total_amount')
			if completed_by:
		...
		if ((filters.get('service_date') and complaint_date and getdate(complaint_date) >= getdate(filters.get("service_date")[0])
